...
{
	sm_simplecsgoantiafk_kicktype = CreateConVar("sm_simplecsgoantiafk_kicktype", "2", "Sets whether to kick to spec (1) or to kick out of match (2)")
...
public Action Event_BombDefused(Handle event, char[] name, bool dontBroadcast) //new
{
	checkPositions();
...
public Action Event_BombExploded(Handle event, char[] name, bool dontBroadcast)
{	
	checkPositions();
	return Plugin_Continue;
}
