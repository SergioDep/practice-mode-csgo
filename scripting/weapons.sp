...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max)
{
	...
	CreateNative("Weapons_CopyClientProps", Weapons_CopyClientProps_Native);
...
void SetWeaponProps(int client, int entity)
{
	...
	if (index > -1 && g_iSkins[target][index] != 0)
	{
		...
		SetEntProp(entity, Prop_Send, "m_nFallbackPaintKit", g_iSkins[target][index] == -1 ? GetRandomSkin(target, index) : g_iSkins[target][index]);
		SetEntPropFloat(entity, Prop_Send, "m_flFallbackWear", !g_bEnableFloat || g_fFloatValue[target][index] == 0.0 ? 0.000001 : g_fFloatValue[target][index] == 1.0 ? 0.999999 : g_fFloatValue[target][index]);
		if (g_bEnableSeed && g_iWeaponSeed[target][index] != -1)
		{
			SetEntProp(entity, Prop_Send, "m_nFallbackSeed", g_iWeaponSeed[target][index]);
		}
		else
		{
			g_iSeedRandom[target][index] = GetRandomInt(0, 8192);
		...
		if(!isKnife)
		{
			...
			{
				SetEntProp(entity, Prop_Send, "m_nFallbackStatTrak", g_iStatTrak[target][index] == 1 ? g_iStatTrakCount[target][index] : -1);
				SetEntProp(entity, Prop_Send, "m_iEntityQuality", g_iStatTrak[target][index] == 1 ? 9 : 0);
			}
		}
		else
		{
			if(g_bEnableStatTrak)
			{
				SetEntProp(entity, Prop_Send, "m_nFallbackStatTrak", g_iStatTrak[target][index] == 0 ? -1 : g_iKnifeStatTrakMode == 0 ? GetTotalKnifeStatTrakCount(target) : g_iStatTrakCount[target][index]);
			}
			SetEntProp(entity, Prop_Send, "m_iEntityQuality", 3);
		}
		if (g_bEnableNameTag && strlen(g_NameTag[target][index]) > 0)
		{
