...
public bool TeleportToSavedGrenadePosition(int client, const char[] id) {
  ...
  DB_TeleportToGrenade(client, g_CurrentSavedGrenadeId[client]);
...
bool ThrowGrenade(int client, const char[] id, float delay = 0.0) {
  if (!g_CSUtilsLoaded) {
    return false;
  }
  int grenadeid = StringToInt(id);
  DB_ThrowGrenade(client, grenadeid);
  return true;
}
  ...
  if (ret < Plugin_Handled) {
    if (g_CSUtilsLoaded) {
      if (!IsGrenade(g_LastGrenadeType[client])) {
        ...
        DB_SaveGrenade(client, origin, angles,
          grenadeType, grenadeOrigin, grenadeVelocity, grenadeDetonationOrigin,
          name, execution
        );
        // // g_UpdatedGrenadeKv = true;
        OnGrenadeKvMutate();
